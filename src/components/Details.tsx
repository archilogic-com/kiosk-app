...
import type { DirectionStep, Route, StepKind } from '#/floor-plan/wayfinding'
import {
  CATEGORY_LABELS,
  KIOSK_COLORS,
  avatarColor,
...
function Pill({ icon: Icon, label }: { icon: LucideIcon; label: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-secondary/60 px-3 py-1.5 text-xs font-medium text-muted-foreground">
...
export function EventInfo({
  ...
  return (
    <>
      <p className="px-5 py-3 text-sm leading-relaxed">{event.description}</p>
      {space && (
...
function BackButton({
  ...
  return (
    <button
      ref={ref}
      onClick={onClick}
      className="flex h-12 w-full shrink-0 items-center gap-2 rounded-t-[inherit] px-5 text-sm font-medium text-muted-foreground outline-hidden transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:ring-inset active:scale-[0.98]"
    >
...
function RouteSummary({ route: { distance, pathError } }: { route: Route }) {
  ...
  return (
    <div className="mt-4 flex items-center gap-2" aria-busy={distance == null}>
      {distance != null ? (
        <>
          <p className="rounded-lg bg-primary/10 px-2.5 py-1 text-sm font-semibold tabular-nums text-primary">
        ...
        <>
          <span className="skeleton h-7 w-14 rounded-lg" />
...
function Directions({
  ...
  return (
    <div className="flex min-h-0 flex-col border-t border-border/30">
      ...
      {open && !loading && (
        <>
          <StepList directions={directions} onHover={onHoverStep} />
...
/** The steps, each lighting up its stretch of the path while the pointer is on it. */
function StepList({
  directions,
  onHover,
}: {
  ...
  // Collapsing the list or leaving the route takes the rows away before the
  // pointer can leave one, so the highlight is cleared on the way out.
  const clear = useEffectEvent(() => onHover(null))
  useEffect(() => clear, [])
  ...
  return (
    <ol className="min-h-0 flex-1 space-y-1 overflow-y-auto px-5 pb-5">
      {directions.map((step, i) => {
        ...
        const tone =
          step.kind === 'arrive' ? 'text-primary' : 'text-muted-foreground'
        return (
          <li
            key={i}
            className="-mx-2 flex items-start gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-accent/50"
...
function CopyLinkButton({ item }: { item: NavigableItem }) {
  ...
  return (
    <button
      onClick={copy}
      className="flex w-full items-center justify-center gap-2 rounded-lg bg-secondary/60 px-4 py-2 text-sm font-medium text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground active:scale-[0.97]"
    >
      {status === 'copied' ? (
