...
export function DashboardSkeleton() {
  return (
    <DashboardColumn>
      <WelcomeHeader />
      <div className="floating-panel grid grid-cols-2 gap-3 p-3">
        <StatCard icon={Users} label="In Office" />
        <StatCard icon={DoorOpen} label="Free Rooms" />
      </div>
      <EventsSummary />
      <WeatherCard />
      <SearchButton />
...
function StatCard({
  ...
  return (
    <button
      onClick={onClick}
      disabled={!onClick}
      className="rounded-xl bg-secondary/60 px-4 py-4 text-left transition-all enabled:hover:bg-secondary enabled:active:scale-[0.97]"
    >
...
function WeatherCard() {
  const now = useNow()
  const { location, currentTempC, condition, hourly } = MOCK_WEATHER
  const Current = WEATHER_ICONS[condition]
  const hourFromNow = (hoursAhead: number) =>
    new Date(now.getTime() + hoursAhead * 3_600_000).toLocaleTimeString(
      'en-US',
      { hour: 'numeric' },
    )
  return (
    <div className="floating-panel bg-sky-50/60 px-6 py-5">
      ...
      <div className="flex gap-4">
        {hourly.map(({ hoursAhead, tempC, condition }) => {
          ...
          return (
            ...
            >
              <span className="text-xs text-muted-foreground">
                {hourFromNow(hoursAhead)}
              </span>
              <Icon className="size-5 text-sky-400" aria-hidden="true" />
              <span className="text-sm font-medium tabular-nums">{tempC}°</span>
            </div>
          )
        })}
      </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 px-5 pt-4 pb-3">
        ...
        <span className="ml-auto rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-semibold tabular-nums text-primary">
          {events.length}
        </span>
      </div>
      ...
      <div className="kiosk-scroll max-h-[65vh] overflow-y-auto px-5 pb-5">
        <div className="relative">
          <div
            className="absolute top-3 bottom-3 left-[27px] w-px bg-border/60"
            aria-hidden="true"
          />
