...
describe('floorPlanStyle', () => {
  ...
  it('draws boundary walls and columns in the wall colour, as the palette describes them', () => {
    const wall = DEFAULT_STYLES['element:wall']
    for (const type of ['element:boundaryWall', 'element:column'] as const) {
      expect(DEFAULT_STYLES[type]).toMatchObject({
        fill: wall.fill,
