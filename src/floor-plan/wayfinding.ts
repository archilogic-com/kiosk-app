...
const STEP_LAYER_ID = 'wayfinding-step'
/** How much wider than the path the band under a hovered step is. */
const STEP_HIGHLIGHT_SCALE = 3
const STEP_HIGHLIGHT_OPACITY = 0.25
/** How far along the path either side of a turn its highlight reaches. */
const STEP_HIGHLIGHT_REACH_M = 2
...
/** A soft band over the stretch of path a direction step covers. */
export function useStepHighlight(
  ...
  useEffect(() => {
    if (!step) return
    const layer = floorPlan.addLayer({ id: STEP_LAYER_ID })
    layer.addGraphic({
      shapes: [
        {
          type: 'curve:polyline',
          points: roundCorners(step.points, CORNER_RADIUS_M, style.smoothing),
          style: {
            stroke: KIOSK_COLORS.path,
            strokeWidth: style.thickness * STEP_HIGHLIGHT_SCALE,
            strokeOpacity: STEP_HIGHLIGHT_OPACITY,
          },
        },
      ],
    })
    return () => layer.destroy()
  }, [floorPlan, step, style.smoothing, style.thickness])
}
...
export interface DirectionStep {
  ...
  /** The stretch of the drawn path this step covers, for the plan to highlight. */
  points: Vector2[]
}
...
/** The indices of the points where the path really changes direction. */
function simplifyPath(path: Vector2[]): number[] {
  if (path.length <= 2) return path.map((_, i) => i)
  const kept = [0]
  for (let i = 1; i < path.length - 1; i++) {
    const angle = turnAngle(path[kept[kept.length - 1]], path[i], path[i + 1])
    if (Math.abs(angle) >= STRAIGHT_THRESHOLD) kept.push(i)
  }
  kept.push(path.length - 1)
  return kept
}

/**
 * The stretch of `path` within `reach` metres of the point at `index`,
 * following the path's own vertices so it lies on the drawn route.
 */
function stretchAround(
  path: Vector2[],
  index: number,
  reach: number,
): Vector2[] {
  const walk = (direction: -1 | 1): Vector2[] => {
    const points: Vector2[] = []
    let left = reach
    for (let i = index; left > 0; i += direction) {
      const from = path[i]
      const to = path[i + direction]
      if (!to) break
  ...
  return [...walk(-1).reverse(), path[index], ...walk(1)]
}
...
export function generateDirections(
  ...
  const kept = simplifyPath(rawPath)
  const path = kept.map((i) => rawPath[i])
  ...
  const pushWalk = () => {
    ...
    const [from, to] = [path[walkFrom], path[walkTo]]
    ...
    steps.push({
      ...
      distance: walked,
      landmark: null,
      points: rawPath.slice(kept[walkFrom], kept[walkTo] + 1),
    })
  }
  ...
  for (let i = 1; i < path.length - 1; i++) {
    ...
    if (!previous || previous.kind === 'walk') {
      ...
      steps.push({
        ...
        distance: 0,
        landmark,
        points: stretchAround(rawPath, kept[i], STEP_HIGHLIGHT_REACH_M),
      })
    }
  ...
  steps.push({
    ...
    points: stretchAround(rawPath, rawPath.length - 1, STEP_HIGHLIGHT_REACH_M),
  })
  return steps
}
