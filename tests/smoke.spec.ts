...
import { diffRatio, Kiosk, TEXAS } from './kiosk'
...
test.describe('kiosk', () => {
  ...
  test('finds spaces, people and events', async ({ page }) => {
    ...
    await kiosk.search('Texas')
    await expect(kiosk.results()).toHaveCount(1)
    await expect(kiosk.result('Texas')).toContainText('Meeting Room')
  ...
  test('typing with a category chip active keeps the first keystroke', async ({
    ...
    await kiosk.searchInput.pressSequentially('Te', { delay: 80 })
    await expect(kiosk.searchInput).toHaveValue('Te')
    await expect(kiosk.result('Texas')).toBeVisible()
  })
  ...
  test('selecting a destination draws a path with distance and directions', async ({
    ...
    await kiosk.enterSearch()
    await kiosk.search('Texas')
    await kiosk.result('Texas').click()

    const panel = page.getByRole('region', { name: /directions to Texas/i })
    ...
    await panel
      ...
      .click()
    await expect(panel).toContainText(/Arrive at Texas/)
  ...
  test('changing destination leaves no orphaned markers or layers', async ({
    ...
    const walks: string[] = []
    for (const name of ['Texas', 'Arizona', 'Georgia']) {
  ...
  test('hover previews a space and releasing restores it exactly', async ({
    ...
    await kiosk.result('Arizona').hover()
    await kiosk.settle()
  ...
  test('a deep link opens on the destination and clears its own parameters', async ({
    ...
    const kiosk = new Kiosk(page)
    await kiosk.open(`?to=${TEXAS}&type=space&directions=1`)
    ...
    await expect(
      page.getByRole('region', { name: /directions to Texas/i }),
    ).toBeVisible()
    await expect(page.getByText('Arrive at Texas')).toBeVisible()
